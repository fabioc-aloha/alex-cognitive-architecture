...
The primary blue button at the top opens Copilot Chat with Alex ready to help. This is the fastest way to start any conversation — no prompt template, just a blank chat.
...
To add a button to an existing group, edit `.github/config/loop-menu.json`:
...
Prompt templates are loaded once per session and cached. If you edit a prompt file, click the **Refresh** button (or reload the sidebar) to pick up changes.
...
Frecency data is stored in VS Code global state. To reset, run the command palette (`Ctrl+Shift+P`) → "Developer: Reset User Data" — but this resets all VS Code state, not just Alex. There's no button-specific reset.
