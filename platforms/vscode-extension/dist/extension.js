"use strict";var Mt=Object.create;var oe=Object.defineProperty;var Ft=Object.getOwnPropertyDescriptor;var Ot=Object.getOwnPropertyNames;var Lt=Object.getPrototypeOf,Gt=Object.prototype.hasOwnProperty;var Nt=(e,t)=>{for(var n in t)oe(e,n,{get:t[n],enumerable:!0})},Ne=(e,t,n,o)=>{if(t&&typeof t=="object"||typeof t=="function")for(let s of Ot(t))!Gt.call(e,s)&&s!==n&&oe(e,s,{get:()=>t[s],enumerable:!(o=Ft(t,s))||o.enumerable});return e};var m=(e,t,n)=>(n=e!=null?Mt(Lt(e)):{},Ne(t||!e||!e.__esModule?oe(n,"default",{value:e,enumerable:!0}):n,e)),Ut=e=>Ne(oe({},"__esModule",{value:!0}),e);var eo={};Nt(eo,{activate:()=>Xn,deactivate:()=>Zn});module.exports=Ut(eo);var l=m(require("vscode"));var Te=m(require("vscode")),ce=m(require("path")),le=m(require("fs"));var g=m(require("fs")),x=m(require("path")),C=m(require("vscode"));function A(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function b(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;").replace(/`/g,"&#96;").replace(/\\/g,"&#92;")}var N=m(require("fs")),Se=m(require("path")),_t=".agent-metrics-state.json",Ht=2160*60*60*1e3;function Ue(e){return Se.join(e,_t)}function _e(e){return Se.join(e,".agent-metrics-runlog.json")}function Bt(e){try{return JSON.parse(N.readFileSync(Ue(e),"utf-8"))}catch{return{}}}function Wt(e,t){N.writeFileSync(Ue(e),JSON.stringify(t,null,2)+`
`,"utf-8")}function zt(e){try{return JSON.parse(N.readFileSync(_e(e),"utf-8"))}catch{return{runs:[]}}}function qt(e,t){N.writeFileSync(_e(e),JSON.stringify(t,null,2)+`
`,"utf-8")}function Jt(e){let t=Date.now()-Ht;return{runs:e.runs.filter(n=>n.completedAt>t)}}var xe=new Map;function se(e){let t=`${e}:${Date.now()}`;return xe.set(t,{startedAt:Date.now()}),t}function j(e,t,n){let o=xe.get(t),s=Date.now(),r=zt(e);r.runs.push({taskId:t.split(":")[0],startedAt:o?.startedAt??s,completedAt:s,success:n});let i=Jt(r);qt(e,i),xe.delete(t),Vt(e,i)}function Vt(e,t){let n=Bt(e),o=new Date().toISOString(),s=t.runs;if(n["tasks-run-count"]={value:s.length,lastUpdated:o},s.length>0){let i=s.filter(a=>a.success).length;n["task-success-rate"]={value:Math.round(i/s.length*100),lastUpdated:o}}let r=s.map(i=>(i.completedAt-i.startedAt)/1e3).filter(i=>i>0);if(r.length>0){let i=r.reduce((a,p)=>a+p,0)/r.length;n["task-duration"]={value:Math.round(i),lastUpdated:o}}n["loop-guard-triggers"]||(n["loop-guard-triggers"]={value:0,lastUpdated:o}),Wt(e,n)}function Yt(e){let t=e.split(/\s+/);if(t.length!==5)return e;let[n,o,s,r,i]=t,a=`${o.padStart(2,"0")}:${n.padStart(2,"0")} UTC`;return s.startsWith("*/")&&r==="*"&&i==="*"?`Every ${s.slice(2)} days at ${a}`:o.startsWith("*/")?`Every ${o.slice(2)} hours`:s==="*"&&r==="*"&&i==="*"&&!o.includes("/")&&!o.includes(",")?`Daily at ${a}`:s==="*"&&r==="*"&&i!=="*"?`${{0:"Sun",1:"Mon",2:"Tue",3:"Wed",4:"Thu",5:"Fri",6:"Sat"}[i]??i} at ${a}`:e}function Kt(e){let t=Date.now()-new Date(e).getTime(),n=Math.floor(t/6e4);if(n<1)return"just now";if(n<60)return`${n}m ago`;let o=Math.floor(n/60);return o<24?`${o}h ago`:`${Math.floor(o/24)}d ago`}function Y(e){let t=x.join(e,".git","config");if(g.existsSync(t))try{let n=g.readFileSync(t,"utf-8"),o=n.match(/url\s*=\s*https:\/\/github\.com\/([^/\s]+\/[^/\s.]+)/);if(o)return`https://github.com/${o[1]}`;let s=n.match(/url\s*=\s*git@github\.com:([^/\s]+\/[^/\s.]+)/);return s?`https://github.com/${s[1]}`:void 0}catch{return}}var Qt=x.join(".github","config",".scheduled-tasks-state.json");function He(e){return x.join(e,Qt)}function Ae(e){try{return JSON.parse(g.readFileSync(He(e),"utf-8"))}catch{return{}}}function Be(e,t){let n=Ae(e);n[t]={lastRun:new Date().toISOString()};let o=He(e),s=x.dirname(o);g.existsSync(s)||g.mkdirSync(s,{recursive:!0}),g.writeFileSync(o,JSON.stringify(n,null,2)+`
`,"utf-8")}var re=null,We="alex.scheduledRuns";function ze(e){re=e}function Ce(){return re?.get(We)??{}}function qe(e){re&&re.update(We,e)}function Je(e){return Ce()[e]}function Ve(e){let t=Ce();delete t[e],qe(t)}function ie(e,t){let n=Ce();n[e]=t,qe(n)}function Xt(e,t,n){if(!e.dependsOn||e.dependsOn.length===0)return{satisfied:!0,blocking:[]};let o=Ae(n),s=[];for(let r of e.dependsOn){if(!t.find(p=>p.id===r)){s.push(`${r} (not found)`);continue}let a=Je(r);if(a){a.status==="failure"||a.status==="error"||a.status==="cancelled"?s.push(`${r} (${a.status})`):a.status!=="completed"&&s.push(`${r} (${a.status})`);continue}o[r]?.lastRun||s.push(`${r} (never run)`)}return{satisfied:s.length===0,blocking:s}}function Ye(e){let t=new Map(e.map(i=>[i.id,i])),n=new Set,o=new Set,s=[];function r(i){if(n.has(i)||o.has(i))return;o.add(i);let a=t.get(i);if(a?.dependsOn)for(let p of a.dependsOn)t.has(p)&&r(p);o.delete(i),n.add(i),a&&s.push(a)}for(let i of e)r(i.id);for(let i of e)n.has(i.id)||s.push(i);return s}function Zt(e){let t=e.match(/github\.com\/([^/]+)\/([^/]+)/);if(t)return{owner:t[1],repo:t[2]}}async function ae(e,t,n,o){let s=Zt(e);if(!s)throw new Error("Cannot parse GitHub owner/repo from URL");let r=se(t),i=o??C.workspace.workspaceFolders?.[0]?.uri.fsPath,{owner:a,repo:p}=s,d=`scheduled-${t}.yml`,y={Authorization:`Bearer ${(await C.authentication.getSession("github",["repo"],{createIfNone:!0})).accessToken}`,Accept:"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28","User-Agent":"alex-cognitive-architecture"};ie(t,{status:"queued"}),n({status:"queued"});let L=new Date().toISOString(),F=`https://api.github.com/repos/${a}/${p}/actions/workflows/${d}/dispatches`,J=await fetch(F,{method:"POST",headers:{...y,"Content-Type":"application/json"},body:JSON.stringify({ref:"main"})});if(!J.ok){let O=await J.text(),ke={status:"error"};throw ie(t,ke),n(ke),i&&j(i,r,!1),new Error(`Dispatch failed (${J.status}): ${O}`)}let be=`https://api.github.com/repos/${a}/${p}/actions/workflows/${d}/runs?per_page=1&created=>${L.slice(0,19)}Z`,te=0,ve=120,ye=5e3,G=async()=>{if(te++,te>ve){let O={status:"error"};ie(t,O),n(O),i&&j(i,r,!1);return}try{let O=await fetch(be,{headers:y});if(!O.ok)return ne();let D=(await O.json()).workflow_runs?.[0];if(!D)return ne();let Ge={status:D.status==="completed"?D.conclusion==="success"?"completed":D.conclusion??"failure":D.status??"in_progress",runUrl:D.html_url,conclusion:D.conclusion??void 0};if(ie(t,Ge),n(Ge),D.status!=="completed")return ne();i&&j(i,r,D.conclusion==="success")}catch{return ne()}},V=!1,we=[],ne=()=>{V||we.push(setTimeout(()=>void G(),ye))};return we.push(setTimeout(()=>void G(),3e3)),{dispose:()=>{V=!0,we.forEach(clearTimeout)}}}function _(e){let t=x.join(e,".github","config","scheduled-tasks.json");if(!g.existsSync(t))return[];try{return JSON.parse(g.readFileSync(t,"utf-8")).tasks??[]}catch{return[]}}function Ke(e,t){let n=x.join(e,".github","config","scheduled-tasks.json");if(!g.existsSync(n))return null;try{let o=JSON.parse(g.readFileSync(n,"utf-8")),s=o.tasks?.find(r=>r.id===t);return s?(s.enabled=!s.enabled,g.writeFileSync(n,JSON.stringify(o,null,2)+`
`,"utf-8"),s.enabled?nt(e,s):ot(e,s.id),o.tasks):null}catch{return null}}function Qe(e,t){let n=x.join(e,".github","config","scheduled-tasks.json");if(!g.existsSync(n))return null;try{let o=JSON.parse(g.readFileSync(n,"utf-8")),s=o.tasks?.findIndex(r=>r.id===t);return s===void 0||s<0?null:(o.tasks.splice(s,1),g.writeFileSync(n,JSON.stringify(o,null,2)+`
`,"utf-8"),ot(e,t),o.tasks)}catch{return null}}function Pe(e,t){let n=x.join(e,".github","workflows",`scheduled-${t}.yml`);return g.existsSync(n)}function Xe(e){return x.join(e,".github","workflows")}function Ze(e,t){return x.join(Xe(e),`scheduled-${t}.yml`)}function U(e){return e.replace(/[\\"`$!#&|;(){}<>]/g,"")}function et(e){if(!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(e))throw new Error(`Invalid task ID: "${e}". Must be lowercase alphanumeric with hyphens.`);return e}function tt(e){if(!/^[0-9*\/,\- ]+$/.test(e))throw new Error(`Invalid cron expression: "${e}".`);return e}function en(e){let t=U(e.name),n=et(e.id),o=tt(e.schedule);return`# Auto-generated \u2014 do not edit manually
name: "Scheduled: ${t}"
...
`}function tn(e){let t=U(e.name),n=U(e.description),o=e.muscle?U(e.muscle):"",s=e.muscleArgs?` ${e.muscleArgs.map(U).join(" ")}`:"",r=et(e.id),i=tt(e.schedule);return`# Auto-generated \u2014 do not edit manually
name: "Scheduled: ${t}"
      ...
      - name: Create PR if changes exist
        ...
        run: |
          ...
          gh pr create \\
            ...
            --label automated \\
            --base main
`}function nt(e,t){let n;if(t.mode==="agent"&&t.promptFile)n=en(t);else if(t.mode==="direct"&&t.muscle)n=tn(t);else return!1;let o=Xe(e);return g.existsSync(o)||g.mkdirSync(o,{recursive:!0}),g.writeFileSync(Ze(e,t.id),n,"utf-8"),!0}function ot(e,t){let n=Ze(e,t);g.existsSync(n)&&g.unlinkSync(n)}function st(e,t,n){let o;if(e.length===0)o=`
    ...
    <div class="schedule-tasks">
      ${e.map(i=>{let a=i.mode==="agent"?"hubot":"terminal",p=i.mode==="agent"?"Agent":"Script",d=Yt(i.schedule),c=n?Pe(n,i.id):!1,u=Je(i.id),y;u&&(u.status==="queued"||u.status==="in_progress")?y='<span class="schedule-pill schedule-pill-running"><span class="codicon codicon-loading codicon-modifier-spin"></span> Running</span>':u&&u.status==="completed"?y='<span class="schedule-pill schedule-pill-success"><span class="codicon codicon-check"></span> Passed</span>':u&&(u.status==="failure"||u.status==="cancelled"||u.status==="error")?y='<span class="schedule-pill schedule-pill-fail"><span class="codicon codicon-error"></span> Failed</span>':y=i.enabled?'<span class="schedule-pill schedule-pill-on">Active</span>':'<span class="schedule-pill schedule-pill-off">Paused</span>';let L=u&&(u.status==="queued"||u.status==="in_progress"),F=i.mode==="agent"&&i.promptFile?L?'<button class="schedule-action-btn schedule-action-running" disabled title="Running\u2026"><span class="codicon codicon-loading codicon-modifier-spin"></span></button>':`<button class="schedule-action-btn schedule-action-run" data-command="runTask" data-file="${b(i.id)}" title="${c?"Run on GitHub Actions":"Run now in Copilot Chat"}">\u{1F680}</button>`:"",J=u?.runUrl?`<button class="schedule-action-btn" data-command="openExternal" data-file="${b(u.runUrl)}" title="View run on GitHub"><span class="codicon codicon-link-external"></span></button>`:"",be=i.mode==="agent"&&i.promptFile?`<button class="schedule-action-btn" data-command="openPromptFile" data-file="${b(i.promptFile)}" title="Edit prompt"><span class="codicon codicon-edit"></span></button>`:"",te=`<button class="schedule-action-btn ${i.enabled?"schedule-action-pause":"schedule-action-resume"}" data-command="toggleTask" data-file="${b(i.id)}" title="${i.enabled?"Pause":"Resume"}">${i.enabled?'<span class="codicon codicon-debug-pause"></span>':'<span class="codicon codicon-play-circle"></span>'}</button>`,ve=`<button class="schedule-action-btn schedule-action-danger" data-command="deleteTask" data-file="${b(i.id)}" title="Delete"><span class="codicon codicon-trash"></span></button>`,ye=s[i.id]?.lastRun?`<span class="schedule-last-run" title="Last run: ${b(s[i.id].lastRun)}"><span class="codicon codicon-history"></span> ${Kt(s[i.id].lastRun)}</span>`:"",G="";if(i.dependsOn&&i.dependsOn.length>0&&n){let V=Xt(i,e,n);V.satisfied?G='<div class="schedule-task-deps schedule-deps-ok"><span class="codicon codicon-pass"></span> Dependencies met</div>':G=`<div class="schedule-task-deps schedule-deps-blocked"><span class="codicon codicon-lock"></span> Blocked by: ${A(V.blocking.join(", "))}</div>`}return`
      <div class="schedule-task ${i.enabled?"enabled":"disabled"}" data-task-id="${b(i.id)}">
        <div class="schedule-task-header">
          ...
          <span class="schedule-task-name" title="${b(i.name)}">${A(i.name)}</span>
          ${y}
        </div>
...
`;var at=async(e,t,n,o)=>e.command==="autopilot"?sn(e,n):e.command==="cloud"?rn(e,n,o):{};async function sn(e,t){let n=Te.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!n)return t.markdown("Open a workspace folder to use Autopilot."),{};let o=_(n),s=e.prompt.trim().toLowerCase();if(s==="list"||s===""){t.markdown(`## Autopilot Tasks

`);let r=Ye(o);for(let i of r){let a=i.mode==="agent"?"\u{1F916}":"\u2699\uFE0F",p=i.enabled?"Active":"Paused",d=i.dependsOn?.length?` \u2190 depends on: ${i.dependsOn.join(", ")}`:"";t.markdown(`- ${a} **${i.name}** \u2014 ${p}${d}
`)}return t.markdown(`
...
`),t.markdown("- `/autopilot list` \u2014 Show all configured tasks\n"),t.markdown("- `/autopilot status` \u2014 Show active tasks and schedules\n"),t.markdown("- `/autopilot run <task-id>` \u2014 Run a specific task\n"),{}}async function rn(e,t,n){let o=Te.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!o)return t.markdown("Open a workspace folder to use cloud dispatch."),{};let s=_(o),r=e.prompt.trim().toLowerCase();if(r===""||r==="help")return t.markdown(`## Cloud Dispatch
...
`);try{let d=await ae(p,a.id,()=>{},o);n.onCancellationRequested(()=>d.dispose()),t.markdown("Workflow dispatched. Check the Autopilot tab or GitHub Actions for progress.")}catch(d){let u=(d instanceof Error?d.message:String(d)).replace(/[A-Z]:\\[\w\\.\.\-\s]+/gi,"[path]").replace(/\/(?:home|usr|tmp|var|etc|Users|mnt)\/[\w/.\.\-]+/g,"[path]");t.markdown(`Dispatch failed: ${u}`)}return{}}return t.markdown("Unknown cloud command. Use `/cloud help` for usage."),{}}var f=m(require("vscode")),Pt=m(require("crypto")),M=m(require("fs")),k=m(require("path")),W=m(require("os"));var E=m(require("fs")),$=m(require("path")),pe=require("child_process");function de(e,t){return Math.abs(t.getTime()-e.getTime())/(1e3*60*60*24)}function an(e){let t=new Date,n=e.lastDreamDate?Math.floor(de(e.lastDreamDate,t)):1/0;return e.dreamNeeded&&n>14||e.syncStale&&n>3?"critical":e.dreamNeeded||n>7||e.syncStale?"attention":"healthy"}function cn(e){let t=$.join(e,".github","quality","dream-report.json");try{let n=E.readFileSync(t,"utf-8");return JSON.parse(n)}catch{return null}}function ln(e){let t=$.join(e,".github",".sync-manifest.json");try{let n=E.readFileSync(t,"utf-8");return JSON.parse(n)}catch{return null}}function dn(){try{let e=$.resolve(__dirname,"..","package.json");return JSON.parse(E.readFileSync(e,"utf-8")).version??"0.0.0"}catch{return"0.0.0"}}function pn(e){let t=ln(e);if(!t)return!1;let n=dn();if(t.brainVersion!==n)return!0;let o=new Date(t.lastSync);return!!(isNaN(o.getTime())||de(o,new Date)>7)}function un(e){try{let t=(0,pe.execFileSync)("git",["diff","--cached","--name-only"],{cwd:e,encoding:"utf-8",timeout:5e3}).trim(),n=(0,pe.execFileSync)("git",["diff","--name-only"],{cwd:e,encoding:"utf-8",timeout:5e3}).trim(),o=t?t.split(`
`).length:0,s=n?n.split(`
`).length:0,r=o+s;if(r===0)return{fileCount:0,days:0};let i=0;try{let a=(0,pe.execFileSync)("git",["log","-1","--format=%ci"],{cwd:e,encoding:"utf-8",timeout:5e3}).trim();a&&(i=Math.floor(de(new Date(a),new Date)))}catch{}return{fileCount:r,days:i}}catch{return{fileCount:0,days:0}}}function gn(e){let t=$.join(e,".github"),n=s=>{try{return E.readdirSync(s,{withFileTypes:!0}).filter(r=>r.isDirectory()).length}catch{return 0}},o=(s,r,i=!1)=>{try{if(!i)return E.readdirSync(s).filter(d=>d.endsWith(r)).length;let a=0,p=d=>{for(let c of E.readdirSync(d,{withFileTypes:!0}))c.isDirectory()?p($.join(d,c.name)):c.name.endsWith(r)&&a++};return p(s),a}catch{return 0}};return{skills:n($.join(t,"skills")),instructions:o($.join(t,"instructions"),".instructions.md"),prompts:o($.join(t,"prompts"),".prompt.md",!0),agents:o($.join(t,"agents"),".agent.md")}}function ct(e){let t=cn(e),n=gn(e),o=t?new Date(t.timestamp):null,s=t?t.brokenRefs.length>0||t.trifectaIssues.length>20:!0,r=un(e),i={status:"healthy",skillCount:n.skills,instructionCount:n.instructions,promptCount:n.prompts,agentCount:n.agents,lastDreamDate:o,dreamNeeded:s,syncStale:pn(e),uncommittedFileCount:r.fileCount,uncommittedDays:r.days};return i.status=an(i),i}function fn(e,t){let n=(t.getTime()-e.getTime())/864e5,o=Math.LN2/7;return Math.exp(-o*n)}function hn(e,t=new Date){let n=new Date(e.lastUsed),o=fn(n,t);return Math.sqrt(e.count)*o}function lt(e,t,n=new Date){return e.actions.find(s=>s.id===t)?{...e,actions:e.actions.map(s=>s.id===t?{...s,count:s.count+1,lastUsed:n.toISOString()}:s)}:{...e,actions:[...e.actions,{id:t,count:1,lastUsed:n.toISOString()}]}}function dt(e,t,n=new Date){let o=new Map;for(let i of t.actions){let a=hn(i,n);a>=.1&&o.set(i.id,a)}let s=e.filter(i=>o.has(i)),r=e.filter(i=>!o.has(i));return s.sort((i,a)=>(o.get(a)??0)-(o.get(i)??0)),[...s,...r]}function pt(){return{version:1,actions:[]}}var mn=["Two minds, one vision","Better together than alone","Your ideas, amplified","Where human meets machine","The sum of us","Let's build something meaningful","From thought to reality","Creating what neither could alone","Turning possibility into code","What will we make today?","Exploring the edge of possible","Learning never looked like this","Growing smarter, together","Every question opens a door","Uncharted territory, good company","I see what you're building","Your vision, our journey","Thinking alongside you","Understanding before answering"],bn=["Still here, still building","Every pause is a chance to reflect","The best work takes time","Progress isn't always visible","Let's pick up where we left off","Good things are worth the wait","One conversation at a time"],vn=["Fresh starts are powerful","We'll figure this out","The first step is showing up","Building begins with connection","Ready when you are","Together, we'll find the way"],yn=["Two minds, ready to build","Let's see what we can create","The beginning of something","Your partner in possibility"],wn=["A new day to create","Fresh ideas await","What will we build today?"],kn=["Wrapping up what we started","Good work deserves reflection","Tomorrow we build on today"];function xn(e=new Date){let t=e.getHours();return t<12?"morning":t<18?"afternoon":"evening"}function ut(e=new Date){let t=new Date(e.getFullYear(),0,0),n=e.getTime()-t.getTime(),o=1e3*60*60*24;return Math.floor(n/o)}function ue(e,t=new Date){let n=ut(t);return e[n%e.length]}function Sn(e){switch(e){case"healthy":return mn;case"attention":return bn;case"critical":return vn;default:return yn}}function gt(e,t,n){let o=n.join(e,".github","config","taglines.json");if(!t.existsSync(o))return null;try{let s=t.readFileSync(o,"utf-8"),r=JSON.parse(s);return!r.taglines?.project||r.taglines.project.length===0?null:r}catch{return null}}function An(e){let t=[];for(let n of Object.keys(e.taglines)){let o=e.taglines[n];Array.isArray(o)&&t.push(...o)}return t}function ft(e={}){let{status:t="unknown",config:n,useTimeOfDay:o=!0,date:s=new Date}=e,r=ut(s);if(n&&t==="healthy"){let a=n.rotation?.projectWeight??50,p=Math.floor(a);if(r%100<p){let c=An(n);if(c.length>0)return ue(c,s)}}if(o&&t==="healthy"&&r%10===0){let a=xn(s);if(a==="morning")return ue(wn,s);if(a==="evening")return ue(kn,s)}let i=Sn(t);return ue(i,s)}var I=m(require("fs")),H=m(require("path"));function Cn(e){let t=e.match(/^---\r?\n[\s\S]*?\r?\n---\r?\n?/);return t?e.slice(t[0].length).trim():e.trim()}function Pn(e){let t=H.join(e,".github","skills");if(!I.existsSync(t))return[];let n=[],o;try{o=I.readdirSync(t,{withFileTypes:!0})}catch{return[]}for(let s of o){if(!s.isDirectory())continue;let r=H.join(t,s.name,"loop-config.partial.json");if(I.existsSync(r))try{let i=JSON.parse(I.readFileSync(r,"utf-8"));if(Array.isArray(i.groups))for(let a of i.groups)n.push({...a,source:"skill"})}catch{}}return n}function Tn(e,t){let n=e.map(o=>({...o,buttons:[...o.buttons]}));for(let o of t){let s=n.find(r=>r.id===o.id);if(s){let r=new Set(s.buttons.map(i=>i.id??i.label));for(let i of o.buttons){let a=i.id??i.label;r.has(a)||(s.buttons.push(i),r.add(a))}}else n.push({...o,buttons:[...o.buttons]})}return n}function $n(e,t){return t?e.filter(n=>!n.phase||n.phase.includes(t)).map(n=>{let o=n.buttons.filter(r=>!r.phase||r.phase.includes(t)),s=n.phase?.includes(t);return{...n,buttons:o,collapsed:s?!1:n.collapsed}}).filter(n=>n.buttons.length>0):e}function In(e,t){let n={id:e.id,icon:e.icon,label:e.label,command:e.command,hint:e.hint,tooltip:e.tooltip};if(e.promptFile){let o=H.join(t,e.promptFile);try{I.existsSync(o)&&(n.prompt=Cn(I.readFileSync(o,"utf-8")))}catch{}}return!n.prompt&&e.prompt&&(n.prompt=e.prompt),e.file&&(n.file=e.file),n}function ht(e){let t=H.join(e,".github","config","loop-menu.json");if(!I.existsSync(t))return[];let n;try{n=JSON.parse(I.readFileSync(t,"utf-8"))}catch{return[]}if(!Array.isArray(n.groups))return[];let o=Pn(e),s=Tn(n.groups,o),r=$n(s,n.projectPhase),i=H.join(e,".github","prompts","loop");return r.map(a=>({id:a.id,label:a.label,desc:a.desc,accent:a.accent,icon:a.icon,collapsed:a.collapsed,buttons:a.buttons.map(p=>In(p,i))}))}var ge=m(require("vscode")),wt=require("child_process");var B={recentAgentPRs:[],pendingReviews:[],totalPending:0,lastFetched:null,error:null};function mt(e,t){return new Promise(n=>{let o=e.split(/\s+/);(0,wt.execFile)("gh",o,{cwd:t,timeout:15e3,maxBuffer:1024*1024},(s,r)=>{if(s){n(null);return}n(r.trim())})})}function bt(e){if(!e)return[];try{return JSON.parse(e).map(n=>({number:n.number,title:n.title,url:n.url,state:n.state,author:n.author?.login??"unknown",createdAt:n.createdAt,updatedAt:n.updatedAt,isDraft:n.isDraft??!1,reviewDecision:n.reviewDecision??"",labels:(n.labels??[]).map(o=>o.name)}))}catch{return[]}}var vt="number,title,url,state,author,createdAt,updatedAt,isDraft,reviewDecision,labels";var $e=!1;async function Ie(e,t){if(!$e){$e=!0;try{let[n,o]=await Promise.all([mt(`pr list --author app/copilot-swe-agent --state all --limit 10 --json ${vt}`,e),mt(`pr list --search review-requested:@me --state open --limit 10 --json ${vt}`,e)]),s=bt(n),r=bt(o);B={recentAgentPRs:s,pendingReviews:r,totalPending:r.length,lastFetched:new Date,error:null}}catch(n){B={...B,error:n instanceof Error?n.message:String(n)}}$e=!1,t?.()}}function Rn(e){let t=Date.now()-new Date(e).getTime(),n=Math.floor(t/36e5);if(n<1)return"just now";if(n<24)return`${n}h ago`;let o=Math.floor(n/24);return o===1?"yesterday":`${o}d ago`}function Dn(e){return e.state==="merged"?"git-merge":e.state==="closed"?"close":e.isDraft?"git-pull-request-draft":"git-pull-request"}function jn(e){return e.state==="merged"?"var(--vscode-charts-purple, #a371f7)":e.state==="closed"?"var(--vscode-errorForeground, #f85149)":"var(--vscode-charts-green, #3fb950)"}function kt(e){let t=B;if(t.error||!t.recentAgentPRs.length&&!t.pendingReviews.length)return`
    ...
    <div class="agent-section">
      ...
      </div>`;for(let o of t.pendingReviews)n+=yt(o);n+="</div>"}if(t.recentAgentPRs.length>0){n+=`
    <div class="agent-section">
      <div class="agent-section-header">
        <span class="codicon codicon-hubot"></span>
        <strong>Recent Agent Sessions</strong>
      </div>`;for(let o of t.recentAgentPRs.slice(0,5))n+=yt(o);n+="</div>"}return n+="</div>",n}function yt(e){let t=Dn(e),n=jn(e);return`
  <div class="agent-pr-row" data-command="openExternal" data-file="${b(e.url)}" role="button" tabindex="0">
    ...
    <div class="agent-pr-info">
      ...
      <span class="agent-pr-meta">#${e.number} \xB7 ${A(Rn(e.createdAt))} \xB7 ${A(e.author)}</span>
    </div>
  </div>`}var w=null;function xt(e){return w=ge.window.createStatusBarItem(ge.StatusBarAlignment.Right,50),w.command="alex.openChat",w.name="Alex Agent Activity",e.subscriptions.push(w),w}function Re(e){if(!w)return;if(!e){w.text="$(brain) Alex",w.tooltip="Alex Cognitive Architecture",w.show();return}let t=B.pendingReviews;t.length>0?(w.text=`$(brain) Alex $(bell-dot) ${t.length}`,w.tooltip=`${t.length} PR${t.length===1?"":"s"} awaiting review`):(w.text="$(brain) Alex",w.tooltip="Alex Cognitive Architecture \u2014 no pending reviews"),w.show(),Ie(e,()=>{let n=B.pendingReviews;w&&(n.length>0?(w.text=`$(brain) Alex $(bell-dot) ${n.length}`,w.tooltip=`${n.length} PR${n.length===1?"":"s"} awaiting review`):(w.text="$(brain) Alex",w.tooltip="Alex Cognitive Architecture \u2014 no pending reviews"))})}var St=`
...
`;var En="alex.welcomeView";function De(){let e=W.platform();return e==="win32"?k.join(process.env.APPDATA||k.join(W.homedir(),"AppData","Roaming"),"Code","User"):e==="darwin"?k.join(W.homedir(),"Library","Application Support","Code","User"):k.join(process.env.XDG_CONFIG_HOME||k.join(W.homedir(),".config"),"Code","User")}var Mn=k.resolve(__dirname,"..","package.json"),Tt=(()=>{try{return JSON.parse(M.readFileSync(Mn,"utf-8")).version??"0.0.0"}catch{return"0.0.0"}})(),je="https://github.com/fabioc-aloha/alex-cognitive-architecture/wiki",Fn=[{id:"workspace",label:"WORKSPACE",icon:"folder",desc:"Initialize and upgrade your cognitive architecture",collapsed:!1,buttons:[{icon:"sync",label:"Initialize Workspace",command:"initialize",tooltip:"Install Alex brain files in this workspace",hint:"command"},{icon:"arrow-up",label:"Upgrade Architecture",command:"upgrade",tooltip:"Update to the latest brain architecture",hint:"command"},{icon:"cloud",label:"Setup AI-Memory",command:"setupAIMemory",tooltip:"Find or create the shared AI-Memory knowledge store",hint:"command"}]},{id:"brain-status",label:"BRAIN STATUS",icon:"symbol-structure",desc:"Cognitive architecture health and maintenance",collapsed:!1,buttons:[{icon:"symbol-event",label:"Run Dream Protocol",command:"dream",tooltip:"Autonomous architecture maintenance",hint:"command"},{icon:"heart",label:"Meditate",command:"openChat",prompt:"Run a meditation session \u2014 consolidate knowledge, review recent changes, and strengthen architecture",tooltip:"Knowledge consolidation session",hint:"chat"},{icon:"graph",label:"Self-Actualize",command:"openChat",prompt:"Run a self-actualization assessment \u2014 evaluate architecture completeness, identify growth areas, and plan improvements",tooltip:"Deep self-assessment and growth",hint:"chat"}]},{id:"user-memory",label:"USER MEMORY",icon:"notebook",desc:"Access your persistent memory locations",collapsed:!0,buttons:[{icon:"notebook",label:"Memories",command:"openMemories",tooltip:"Open VS Code user memories folder",hint:"command"},{icon:"edit",label:"User Prompts",command:"openPrompts",tooltip:"Reusable prompt templates",hint:"command"},{icon:"server",label:"MCP Config",command:"openMcpConfig",tooltip:"Model Context Protocol servers",hint:"command"},{icon:"cloud",label:"Copilot Memory (GitHub)",command:"openExternal",file:"https://github.com/settings/copilot",tooltip:"Manage cloud-synced Copilot memory",hint:"link"}]},{id:"environment",label:"ENVIRONMENT",icon:"settings-gear",desc:"Extension settings",collapsed:!0,buttons:[{icon:"settings-gear",label:"Open Extension Settings",command:"openSettings",tooltip:"Configure Alex extension behavior",hint:"command"}]},{id:"learn",label:"LEARN",icon:"book",desc:"Documentation and support resources",collapsed:!0,buttons:[{icon:"book",label:"Documentation",command:"openExternal",file:`${je}`,tooltip:"Full documentation on GitHub Wiki",hint:"link"},{icon:"comment-discussion",label:"User Stories",command:"openExternal",file:`${je}/blog/README`,tooltip:"Real-world examples of working with Alex",hint:"link"},{icon:"mortar-board",label:"Tutorials",command:"openExternal",file:`${je}/tutorials/README`,tooltip:"Step-by-step guides for common tasks",hint:"link"},{icon:"lightbulb",label:"LearnAI Playbooks",command:"openExternal",file:"https://learnai.correax.com/",tooltip:"80 AI playbooks for professional disciplines",hint:"link"},{icon:"bug",label:"Report an Issue",command:"openExternal",file:"https://github.com/fabioc-aloha/alex-cognitive-architecture/issues",tooltip:"Found a bug? Let us know",hint:"link"}]},{id:"about",label:"ABOUT",icon:"info",collapsed:!0,buttons:[{icon:"info",label:`v${Tt}`,command:"noop"},{icon:"person",label:"Publisher: fabioc-aloha",command:"openExternal",file:"https://github.com/fabioc-aloha",hint:"link"},{icon:"law",label:"PolyForm Noncommercial 1.0.0",command:"openExternal",file:"https://github.com/fabioc-aloha/alex-cognitive-architecture/blob/main/LICENSE.md",hint:"link"}]}],On=["https://github.com/","https://marketplace.visualstudio.com/","https://learnai.correax.com/"],At="alex.quickActionFrecency",K=class{constructor(t,n){this.extensionUri=t;this.globalState=n;this.workspaceRoot=f.workspace.workspaceFolders?.[0]?.uri.fsPath??"",this.frecencyData=n.get(At)??pt()}static viewId=En;view;workspaceRoot;frecencyData;disposables=[];dispose(){for(let t of this.disposables)t.dispose();this.disposables.length=0}recordActionUse(t){this.frecencyData=lt(this.frecencyData,t),this.globalState.update(At,this.frecencyData)}loopGroupsCache=null;getLoopGroups(){return this.loopGroupsCache||(this.loopGroupsCache=this.workspaceRoot?ht(this.workspaceRoot):[]),this.loopGroupsCache}renderGroupsWithFrecency(t){let n=t.map(o=>{if(o.id==="creative-loop")return o;let s=o.buttons.map(a=>a.id??a.label.toLowerCase().replace(/\s+/g,"-")),i=dt(s,this.frecencyData).map(a=>o.buttons.find(p=>(p.id??p.label.toLowerCase().replace(/\s+/g,"-"))===a)).filter(a=>a!==void 0);return{...o,buttons:i}});return Ct(n)}refresh(){this.loopGroupsCache=null,this.view&&(this.view.webview.html=this.getHtml(this.view.webview))}resolveWebviewView(t,n,o){this.view=t,t.webview.options={enableScripts:!0,localResourceRoots:[this.extensionUri]},t.webview.html=this.getHtml(t.webview),this.workspaceRoot&&Ie(this.workspaceRoot,()=>this.refresh()),t.webview.onDidReceiveMessage(s=>this.handleMessage(s)),t.onDidChangeVisibility(()=>{t.visible&&this.refresh()})}async handleMessage(t){switch(t.actionId&&this.recordActionUse(t.actionId),t.command){case"openChat":if(t.prompt){let n=t.prompt.endsWith(": ");await f.commands.executeCommand("workbench.action.chat.open",{query:t.prompt,isPartialQuery:n})}else await f.commands.executeCommand("workbench.action.chat.open");break;case"initialize":await f.commands.executeCommand("alex.initialize");break;case"upgrade":await f.commands.executeCommand("alex.upgrade");break;case"setupAIMemory":await f.commands.executeCommand("alex.setupAIMemory");break;case"dream":await f.commands.executeCommand("alex.dream");break;case"openSettings":await f.commands.executeCommand("workbench.action.openSettings","alex");break;case"openMemories":{let n=f.Uri.file(k.join(De(),"globalStorage","github.copilot-chat","memory-tool","memories"));try{await f.commands.executeCommand("revealFileInOS",n)}catch{await f.commands.executeCommand("vscode.openFolder",n,{forceNewWindow:!1})}}break;case"openPrompts":{let n=f.Uri.file(k.join(De(),"prompts"));try{await f.commands.executeCommand("revealFileInOS",n)}catch{await f.commands.executeCommand("vscode.openFolder",n,{forceNewWindow:!1})}}break;case"openMcpConfig":{let n=f.Uri.file(k.join(De(),"mcp.json"));await f.commands.executeCommand("vscode.open",n)}break;case"openExternal":if(t.file){let n=String(t.file);On.some(o=>n.startsWith(o))&&await f.env.openExternal(f.Uri.parse(n))}break;case"refresh":this.refresh();break;case"toggleTask":if(this.workspaceRoot&&t.file){let n=Ke(this.workspaceRoot,t.file);if(n){this.refresh();let o=n.find(s=>s.id===t.file);if(o){let s=o.enabled?"enabled":"disabled",r=o.enabled?"Workflow created.":"Workflow removed.";f.window.showInformationMessage(`Task "${o.name}" ${s}. ${r} Commit & push to activate on GitHub.`)}}}break;case"addTask":this.workspaceRoot&&await it(this.workspaceRoot)&&this.refresh();break;case"deleteTask":this.workspaceRoot&&t.file&&await f.window.showWarningMessage(`Delete task "${t.file}"?`,{modal:!0},"Delete")==="Delete"&&Qe(this.workspaceRoot,t.file)!==null&&(this.refresh(),f.window.showInformationMessage(`Task "${t.file}" deleted. Commit & push to remove the workflow from GitHub.`));break;case"openPromptFile":if(this.workspaceRoot&&t.file){let n=k.resolve(this.workspaceRoot,t.file);if(!n.toLowerCase().startsWith(this.workspaceRoot.toLowerCase()+k.sep)&&n.toLowerCase()!==this.workspaceRoot.toLowerCase())break;M.existsSync(n)?await f.commands.executeCommand("vscode.open",f.Uri.file(n)):f.window.showWarningMessage(`Prompt file not found: ${t.file}`)}break;case"runTask":if(this.workspaceRoot&&t.file){let n=Y(this.workspaceRoot),o=Pe(this.workspaceRoot,t.file);if(Be(this.workspaceRoot,t.file),this.refresh(),n&&o)try{let s=await ae(n,t.file,r=>{this.refresh()},this.workspaceRoot);this.disposables.push(s),f.window.showInformationMessage(`Workflow dispatched for "${t.file}". Monitoring execution\u2026`)}catch(s){let r=s instanceof Error?s.message:String(s);f.window.showErrorMessage(`Failed to dispatch workflow: ${r}`)}else{let s=se(t.file),i=_(this.workspaceRoot).find(a=>a.id===t.file);if(i?.promptFile){let a=k.resolve(this.workspaceRoot,i.promptFile),p=this.workspaceRoot.toLowerCase()+k.sep;if(!a.toLowerCase().startsWith(p))break;if(M.existsSync(a)){let d=M.readFileSync(a,"utf-8").replace(/^---[\s\S]*?---\s*/,"").trim();await f.commands.executeCommand("workbench.action.chat.open",{query:d,isPartialQuery:!1}),j(this.workspaceRoot,s,!0)}else f.window.showWarningMessage(`Prompt file not found: ${i.promptFile}`),j(this.workspaceRoot,s,!1)}else j(this.workspaceRoot,s,!1)}}break;case"clearRunStatus":t.file&&(Ve(t.file),this.refresh());break;case"openScheduleConfig":if(this.workspaceRoot){let n=k.join(this.workspaceRoot,".github","config","scheduled-tasks.json");M.existsSync(n)&&await f.commands.executeCommand("vscode.open",f.Uri.file(n))}break;case"noop":break;case"switchTab":break}}getHtml(t){let n=Ln(),o=t.asWebviewUri(f.Uri.joinPath(this.extensionUri,"dist","codicon.css")),r=(this.workspaceRoot?ct(this.workspaceRoot):null)?.status??"unknown",i=this.workspaceRoot?gt(this.workspaceRoot,M,k):null,a=ft({status:r,config:i});return`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <div id="tab-loop" class="tab-panel active" role="tabpanel" aria-labelledby="tab-btn-loop">
    <!-- Chat CTA -->
    ${$t({icon:"comment-discussion",label:"Chat with Alex",command:"openChat",hint:"chat"},!0)}
  ...
  <div id="tab-autopilot" class="tab-panel" role="tabpanel" aria-labelledby="tab-btn-autopilot">
    ${this.workspaceRoot?kt(this.workspaceRoot):""}
    ${st(this.workspaceRoot?_(this.workspaceRoot):[],this.workspaceRoot?Y(this.workspaceRoot):void 0,this.workspaceRoot)}
  </div>
...
</html>`}};function Ln(){return Pt.randomBytes(16).toString("hex")}function Ct(e){return e.map(t=>{let n=t.collapsed===!1?"expanded":"",o=t.icon?`<span class="codicon codicon-${b(t.icon)}"></span>`:"";return`
...
## Completed
`,"user-profile.json":()=>JSON.stringify({name:"",role:"",preferences:{communication:"direct",codeStyle:"",learningStyle:""},updatedAt:new Date().toISOString()},null,2),"project-registry.json":()=>JSON.stringify({version:1,projects:[],updatedAt:new Date().toISOString()},null,2),"index.json":()=>JSON.stringify({version:1,files:[],updatedAt:new Date().toISOString()},null,2)};function Kn(e){let t=[];v.existsSync(e)||(v.mkdirSync(e,{recursive:!0}),t.push(e));for(let n of Vn){let o=S.join(e,n);v.existsSync(o)||(v.mkdirSync(o,{recursive:!0}),t.push(o))}for(let[n,o]of Object.entries(Yn)){let s=S.join(e,n);if(!v.existsSync(s)){let r=typeof o=="function"?o():o;v.writeFileSync(s,r,"utf-8"),t.push(s)}}return t}async function Qn(){let e=P.workspace.getConfiguration("alex.aiMemory").get("path");if(e&&v.existsSync(e)){let i=await P.window.showInformationMessage(`AI-Memory already configured at: ${e}`,"Use This","Change Location");if(i==="Use This")return e;if(!i)return}let t=Et(),n=Jn(),o=[];for(let i of t)o.push({label:"$(check) "+i,description:"Found \u2014 existing AI-Memory",detail:i});for(let i of n)if(!t.includes(i)){let a=/OneDrive/i.test(i);o.push({label:(a?"$(cloud) ":"$(folder) ")+i,description:a?"OneDrive \u2014 recommended (cloud-synced)":"Local fallback",detail:i})}o.push({label:"$(folder-opened) Browse...",description:"Choose a custom location",detail:"__browse__"});let s=await P.window.showQuickPick(o,{title:"AI-Memory Location",placeHolder:"Where should Alex store shared knowledge?",ignoreFocusOut:!0});if(!s)return;let r;if(s.detail==="__browse__"){let i=await P.window.showOpenDialog({canSelectFolders:!0,canSelectFiles:!1,canSelectMany:!1,openLabel:"Select AI-Memory Folder",title:"Choose AI-Memory Location"});if(!i||i.length===0)return;r=i[0].fsPath,S.basename(r).toLowerCase().includes("ai-memory")||(r=S.join(r,"AI-Memory"))}else r=s.detail;return r}async function he(){let e=await Qn();if(e)try{let t=Kn(e);return await P.workspace.getConfiguration("alex.aiMemory").update("path",e,P.ConfigurationTarget.Global),t.length>0?P.window.showInformationMessage(`AI-Memory initialized at ${e} (${t.length} items created).`):P.window.showInformationMessage(`AI-Memory linked to ${e} (already complete).`),e}catch(t){let o=(t instanceof Error?t.message:String(t)).replace(/[A-Z]:\\[\w\\.\.\-\s]+/gi,"[path]").replace(/\/(?:home|usr|tmp|var|etc|Users|mnt)\/[\w/.\.\-]+/g,"[path]");P.window.showErrorMessage(`AI-Memory setup failed: ${o}`);return}}var T=m(require("vscode")),Oe=m(require("path")),q=m(require("fs")),z=class extends T.TreeItem{constructor(n,o,s,r,i){super(n,s);this.metricId=r;this.children=i;this.description=o,this.tooltip=`${n}: ${o}`}};function Fe(e,t){switch(t){case"percent":return`${e}%`;case"seconds":return`${e}s`;case"count":return`${e}`;default:return`${e} ${t}`}}var me=class{constructor(t){this.workspaceRoot=t}_onDidChangeTreeData=new T.EventEmitter;onDidChangeTreeData=this._onDidChangeTreeData.event;refresh(){this._onDidChangeTreeData.fire(void 0)}getTreeItem(t){return t}getChildren(t){return this.workspaceRoot?t?.children?t.children:t?[]:this.getRootItems():[new z("No workspace","Open a folder",T.TreeItemCollapsibleState.None)]}getRootItems(){let t=[],n=Oe.join(this.workspaceRoot,".github","config","agent-metrics.json");if(!q.existsSync(n))return[new z("Not configured","No agent-metrics.json found",T.TreeItemCollapsibleState.None)];let o;try{o=JSON.parse(q.readFileSync(n,"utf-8"))}catch{return[new z("Config error","Invalid agent-metrics.json",T.TreeItemCollapsibleState.None)]}let s=Oe.join(this.workspaceRoot,".agent-metrics-state.json"),r={};if(q.existsSync(s))try{r=JSON.parse(q.readFileSync(s,"utf-8"))}catch{}let i=o.thresholds??{};for(let a of o.metrics){let p=r[a.id],d=p?Fe(p.value,a.unit):"\u2014",c=i[a.id],u=this.getStatusIcon(a,p?.value,c),y=new z(a.name,d,T.TreeItemCollapsibleState.None,a.id);y.iconPath=new T.ThemeIcon(u);let L=c?`

Warning: ${Fe(c.warning,a.unit)} \xB7 Critical: ${Fe(c.critical,a.unit)}`:"";y.tooltip=new T.MarkdownString(`**${a.name}**

${a.description}

Current: ${d}${L}`),t.push(y)}return t}getStatusIcon(t,n,o){return n===void 0||!o?"circle-outline":t.higherIsBetter===!0||t.id.includes("rate")||t.id==="tasks-run-count"?n<o.critical?"error":n<o.warning?"warning":"check":n>o.critical?"error":n>o.warning?"warning":"check"}};function ee(e){return(e instanceof Error?e.message:String(e)).replace(/[A-Z]:\\[\w\\.\-\s]+/gi,"[path]").replace(/\/(?:home|usr|tmp|var|etc|Users|mnt)\/[\w/.\-]+/g,"[path]")}async function Le(){let e=l.workspace.getConfiguration(),t="github.copilot.chat.copilotMemory.enabled";e.get(t)!==!1&&await e.update(t,!1,l.ConfigurationTarget.Global)}function Xn(e){Le(),ze(e.workspaceState);let t=l.chat.createChatParticipant("alex.chat",at);t.iconPath=l.Uri.joinPath(e.extensionUri,"assets","icon.png"),t.followupProvider={provideFollowups(){return[{prompt:"/autopilot list",label:"List Autopilot Tasks",command:"autopilot"}]}},e.subscriptions.push(t);let n=new K(e.extensionUri,e.globalState);e.subscriptions.push(n),e.subscriptions.push(l.window.registerWebviewViewProvider(K.viewId,n));let o=l.workspace.workspaceFolders?.[0]?.uri.fsPath;xt(e),Re(o);let s=setInterval(()=>{Re(l.workspace.workspaceFolders?.[0]?.uri.fsPath)},300*1e3);e.subscriptions.push({dispose:()=>clearInterval(s)});let r=new me(o);if(e.subscriptions.push(l.window.createTreeView("alex.agentActivity",{treeDataProvider:r})),e.subscriptions.push(l.commands.registerCommand("alex.refreshAgentActivity",()=>{r.refresh()})),o){let c=new l.RelativePattern(o,".agent-metrics-state.json"),u=l.workspace.createFileSystemWatcher(c);u.onDidChange(()=>r.refresh()),u.onDidCreate(()=>r.refresh()),e.subscriptions.push(u)}e.subscriptions.push(l.commands.registerCommand("alex.refreshWelcome",()=>{n.refresh()})),e.subscriptions.push(l.commands.registerCommand("alex.openChat",()=>{l.commands.executeCommand("workbench.action.chat.open")})),e.subscriptions.push(l.commands.registerCommand("alex.dream",async()=>{try{await l.commands.executeCommand("workbench.action.chat.open",{query:"/dream"})}catch(c){l.window.showErrorMessage(`Dream protocol failed: ${ee(c)}`)}})),e.subscriptions.push(l.commands.registerCommand("alex.initialize",async()=>{try{await Le(),Me()||await l.window.showInformationMessage("Alex: Set up AI-Memory for cross-project knowledge sharing?","Setup AI-Memory","Skip")==="Setup AI-Memory"&&await he(),await l.commands.executeCommand("workbench.action.chat.open",{query:"Initialize this workspace with Alex's cognitive architecture"})}catch(c){l.window.showErrorMessage(`Initialize failed: ${ee(c)}`)}})),e.subscriptions.push(l.commands.registerCommand("alex.upgrade",async()=>{try{await Le(),Me()||await l.window.showInformationMessage("Alex: AI-Memory not found. Set it up for cross-project knowledge sharing?","Setup AI-Memory","Skip")==="Setup AI-Memory"&&await he(),await l.commands.executeCommand("workbench.action.chat.open",{query:"Upgrade this workspace's cognitive architecture to the latest version"})}catch(c){l.window.showErrorMessage(`Upgrade failed: ${ee(c)}`)}})),e.subscriptions.push(l.commands.registerCommand("alex.setupAIMemory",async()=>{try{await he(),n.refresh()}catch(c){l.window.showErrorMessage(`AI-Memory setup failed: ${ee(c)}`)}})),e.subscriptions.push(l.commands.registerCommand("alex.generateLoopConfig",async()=>{let c=l.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!c){l.window.showWarningMessage("Alex: Open a workspace folder first.");return}try{let u=fe(c);Dt(c,u)?l.window.showInformationMessage(`Alex: Loop config generated for ${u.projectType} project (${u.conventions.length} conventions detected).`):l.window.showErrorMessage("Alex: Failed to write loop config.")}catch(u){l.window.showErrorMessage(`Alex: Loop config generation failed \u2014 ${ee(u)}`)}})),e.subscriptions.push(l.commands.registerCommand("alex.setProjectPhase",async()=>{let c=l.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!c){l.window.showWarningMessage("Alex: Open a workspace folder first.");return}let u=[{label:"Planning",description:"Ideation, research, and design",value:"planning"},{label:"Active Development",description:"Building features and writing code",value:"active-development"},{label:"Testing",description:"QA, integration tests, and validation",value:"testing"},{label:"Release",description:"Packaging, publishing, and deployment",value:"release"},{label:"Maintenance",description:"Bug fixes, upgrades, and monitoring",value:"maintenance"}],y=await l.window.showQuickPick(u.map(F=>({label:F.label,description:F.description,value:F.value})),{placeHolder:"Select the current project phase"});if(!y)return;jt(c,y.value)?(n.refresh(),l.window.showInformationMessage(`Alex: Project phase set to "${y.label}".`)):l.window.showErrorMessage("Alex: Failed to update project phase. Generate a loop config first.")}));let i=l.workspace.createFileSystemWatcher(new l.RelativePattern(l.workspace.workspaceFolders?.[0]??"",".github/config/loop-menu.json")),a=l.workspace.createFileSystemWatcher(new l.RelativePattern(l.workspace.workspaceFolders?.[0]??"",".github/prompts/loop/*.prompt.md")),p=l.workspace.createFileSystemWatcher(new l.RelativePattern(l.workspace.workspaceFolders?.[0]??"",".github/skills/*/loop-config.partial.json")),d=()=>n.refresh();e.subscriptions.push(i,i.onDidChange(d),i.onDidCreate(d),i.onDidDelete(d),a,a.onDidChange(d),a.onDidCreate(d),a.onDidDelete(d),p,p.onDidChange(d),p.onDidCreate(d),p.onDidDelete(d))}function Zn(){}0&&(module.exports={activate,deactivate});
