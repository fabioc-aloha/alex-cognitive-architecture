...
`,"utf-8")}function un(e){let t=Date.now()-rn;return{runs:e.runs.filter(n=>n.completedAt>t)}}var Ee=new Map;function me(e){let t=`${e}:${Date.now()}`;return Ee.set(t,{startedAt:Date.now()}),t}function V(e,t,n){let o=Ee.get(t),s=Date.now(),i=dn(e);i.runs.push({taskId:t.split(":")[0],startedAt:o?.startedAt??s,completedAt:s,success:n});let a=un(i);pn(e,a),Ee.delete(t),gn(e,a)}function gn(e,t){let n=cn(e),o=new Date().toISOString(),s=t.runs;if(n["tasks-run-count"]={value:s.length,lastUpdated:o},s.length>0){let a=s.filter(r=>r.success).length;n["task-success-rate"]={value:Math.round(a/s.length*100),lastUpdated:o}}let i=s.map(a=>(a.completedAt-a.startedAt)/1e3).filter(a=>a>0);if(i.length>0){let a=i.reduce((r,u)=>r+u,0)/i.length;n["task-duration"]={value:Math.round(a),lastUpdated:o}}n["loop-guard-triggers"]||(n["loop-guard-triggers"]={value:0,lastUpdated:o}),ln(e,n)}function fn(e){try{let t=(0,ve.execFileSync)("gh",["secret","list","--json","name"],{cwd:e,encoding:"utf-8",timeout:1e4,stdio:["pipe","pipe","pipe"]});return JSON.parse(t).some(o=>o.name==="COPILOT_PAT")}catch{return!1}}async function Fe(e){let t=await x.window.showInformationMessage("Autopilot needs a GitHub token (repo secret) to assign Copilot to issues. Set it up now using your existing GitHub CLI auth?",{modal:!0},"Set Up Automatically","I'll Do It Manually");if(t==="I'll Do It Manually")return x.env.openExternal(x.Uri.parse("https://github.com/settings/personal-access-tokens/new")),x.window.showInformationMessage("Create a fine-grained PAT with Issues (read/write) and Contents (read/write) permissions, then run: gh secret set COPILOT_PAT in your repo."),!1;if(t!=="Set Up Automatically")return!1;try{let n=(0,ve.execFileSync)("gh",["auth","token"],{cwd:e,encoding:"utf-8",timeout:1e4,stdio:["pipe","pipe","pipe"]}).trim();return n?((0,ve.execFileSync)("gh",["secret","set","COPILOT_PAT"],{cwd:e,timeout:15e3,input:n,stdio:["pipe","pipe","pipe"]}),x.window.showInformationMessage("COPILOT_PAT secret created. Copilot can now be auto-assigned to issues."),!0):(x.window.showErrorMessage("Could not retrieve GitHub CLI token. Run `gh auth login` first."),!1)}catch{return x.window.showErrorMessage("Failed to set up COPILOT_PAT. Make sure `gh` CLI is installed and you're authenticated (`gh auth login`)."),!1}}function we(e){let t=j.join(e,".git","config");if(v.existsSync(t))try{let n=v.readFileSync(t,"utf-8"),o=n.match(/url\s*=\s*https:\/\/github\.com\/([^/\s]+\/[^/\s.]+)/);if(o)return`https://github.com/${o[1]}`;let s=n.match(/url\s*=\s*git@github\.com:([^/\s]+\/[^/\s.]+)/);return s?`https://github.com/${s[1]}`:void 0}catch{return}}var mn=j.join(".github","config",".scheduled-tasks-state.json");function Ze(e){return j.join(e,mn)}function hn(e){try{return JSON.parse(v.readFileSync(Ze(e),"utf-8"))}catch{return{}}}function et(e,t){let n=hn(e);n[t]={lastRun:new Date().toISOString()};let o=Ze(e),s=j.dirname(o);v.existsSync(s)||v.mkdirSync(s,{recursive:!0}),v.writeFileSync(o,JSON.stringify(n,null,2)+`
`,"utf-8")}var be=null,tt="alex.scheduledRuns";function nt(e){be=e}function ot(){return be?.get(tt)??{}}function st(e){be&&be.update(tt,e)}function it(e){let t=ot();delete t[e],st(t)}function he(e,t){let n=ot();n[e]=t,st(n)}function at(e){let t=new Map(e.map(a=>[a.id,a])),n=new Set,o=new Set,s=[];function i(a){if(n.has(a)||o.has(a))return;o.add(a);let r=t.get(a);if(r?.dependsOn)for(let u of r.dependsOn)t.has(u)&&i(u);o.delete(a),n.add(a),r&&s.push(r)}for(let a of e)i(a.id);for(let a of e)n.has(a.id)||s.push(a);return s}function vn(e){let t=e.match(/github\.com\/([^/]+)\/([^/]+)/);if(t)return{owner:t[1],repo:t[2]}}async function ye(e,t,n,o){let s=vn(e);if(!s)throw new Error("Cannot parse GitHub owner/repo from URL");let i=me(t),a=o??x.workspace.workspaceFolders?.[0]?.uri.fsPath,{owner:r,repo:u}=s,p=`scheduled-${t}.yml`,d={Authorization:`Bearer ${(await x.authentication.getSession("github",["repo"],{createIfNone:!0})).accessToken}`,Accept:"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28","User-Agent":"alex-cognitive-architecture"};he(t,{status:"queued"}),n({status:"queued"});let m=new Date().toISOString(),S=`https://api.github.com/repos/${r}/${u}/actions/workflows/${p}/dispatches`,b=await fetch(S,{method:"POST",headers:{...d,"Content-Type":"application/json"},body:JSON.stringify({ref:"main"})});if(!b.ok){let W=await b.text(),De={status:"error"};throw he(t,De),n(De),a&&V(a,i,!1),new Error(`Dispatch failed (${b.status}): ${W}`)}let I=`https://api.github.com/repos/${r}/${u}/actions/workflows/${p}/runs?per_page=1&created=>${m.slice(0,19)}Z`,y=0,M=120,U=5e3,X=async()=>{if(y++,y>M){let W={status:"error"};he(t,W),n(W),a&&V(a,i,!1);return}try{let W=await fetch(I,{headers:d});if(!W.ok)return K();let z=(await W.json()).workflow_runs?.[0];if(!z)return K();let Ye={status:z.status==="completed"?z.conclusion==="success"?"completed":z.conclusion??"failure":z.status??"in_progress",runUrl:z.html_url,conclusion:z.conclusion??void 0};if(he(t,Ye),n(Ye),z.status!=="completed")return K();a&&V(a,i,z.conclusion==="success")}catch{return K()}},O=!1,F=[],K=()=>{O||F.push(setTimeout(()=>void X(),U))};return F.push(setTimeout(()=>void X(),3e3)),{dispose:()=>{O=!0,F.forEach(clearTimeout)}}}function ae(e){let t=j.join(e,".github","config","scheduled-tasks.json");if(!v.existsSync(t))return[];try{return JSON.parse(v.readFileSync(t,"utf-8")).tasks??[]}catch{return[]}}function rt(e,t){let n=j.join(e,".github","config","scheduled-tasks.json");if(!v.existsSync(n))return null;try{let o=JSON.parse(v.readFileSync(n,"utf-8")),s=o.tasks?.find(i=>i.id===t);return s?(s.enabled=!s.enabled,v.writeFileSync(n,JSON.stringify(o,null,2)+`
...
`}function wn(e){let t=ee(e.name),n=ee(e.description),o=e.muscle?ee(e.muscle):"",s=e.muscleArgs?` ${e.muscleArgs.map(ee).join(" ")}`:"",i=pt(e.id),a=ut(e.schedule);return`# Auto-generated \u2014 do not edit manually
name: "Scheduled: ${t}"
...
`;var vt=async(e,t,n,o)=>e.command==="autopilot"?kn(e,n):e.command==="cloud"?Sn(e,n,o):{};async function kn(e,t){let n=Le.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!n)return t.markdown("Open a workspace folder to use Autopilot."),{};let o=ae(n),s=e.prompt.trim().toLowerCase();if(s==="list"||s===""){t.markdown(`## Autopilot Tasks

`);let i=at(o);for(let a of i){let r=a.mode==="agent"?"\u{1F916}":"\u2699\uFE0F",u=a.enabled?"Active":"Paused",p=a.dependsOn?.length?` \u2190 depends on: ${a.dependsOn.join(", ")}`:"";t.markdown(`- ${r} **${a.name}** \u2014 ${u}${p}
`)}return t.markdown(`
...
`),t.markdown("- `/autopilot list` \u2014 Show all configured tasks\n"),t.markdown("- `/autopilot status` \u2014 Show active tasks and schedules\n"),t.markdown("- `/autopilot run <task-id>` \u2014 Run a specific task\n"),{}}async function Sn(e,t,n){let o=Le.workspace.workspaceFolders?.[0]?.uri.fsPath;if(!o)return t.markdown("Open a workspace folder to use cloud dispatch."),{};let s=ae(o),i=e.prompt.trim().toLowerCase();if(i===""||i==="help")return t.markdown(`## Cloud Dispatch
...
`,"utf-8"),!0}catch{return!1}}var L=h(require("vscode")),C=h(require("fs")),D=h(require("path")),Be=h(require("os"));function Gt(){let e=[],t=Be.homedir();try{for(let o of C.readdirSync(t))if(/^OneDrive/i.test(o)){let s=D.join(t,o,"AI-Memory");C.existsSync(s)&&e.push(s)}}catch{}let n=D.join(t,"Library","CloudStorage");try{if(C.existsSync(n)){for(let o of C.readdirSync(n))if(/^OneDrive/i.test(o)){let s=D.join(n,o,"AI-Memory");C.existsSync(s)&&e.push(s)}}}catch{}for(let o of[D.join(t,"AI-Memory"),D.join(t,".alex","AI-Memory")])C.existsSync(o)&&e.push(o);return e}function ro(){let e=[],t=Be.homedir();try{for(let o of C.readdirSync(t))/^OneDrive/i.test(o)&&e.push(D.join(t,o,"AI-Memory"))}catch{}let n=D.join(t,"Library","CloudStorage");try{if(C.existsSync(n))for(let o of C.readdirSync(n))/^OneDrive/i.test(o)&&e.push(D.join(n,o,"AI-Memory"))}catch{}return e.push(D.join(t,"AI-Memory")),e.push(D.join(t,".alex","AI-Memory")),e}function _t(){let e=L.workspace.getConfiguration("alex.aiMemory").get("path");if(e&&C.existsSync(e))return e;let t=Gt();return t.length>0?t[0]:null}var co=[".github","announcements","feedback","insights","knowledge","patterns"],lo={"global-knowledge.md":`# Global Knowledge
