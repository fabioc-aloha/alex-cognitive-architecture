...
async function setupCopilotPAT(workspaceRoot) {
  ...
  try {
    ...
    (0, import_child_process.execFileSync)("gh", ["secret", "set", "COPILOT_PAT", "--body", token], {
      cwd: workspaceRoot,
      timeout: 15e3,
      stdio: ["pipe", "pipe", "pipe"]
    });
...
function checkDependencies(task, allTasks, workspaceRoot) {
...
function renderScheduledTasks(tasks, repoUrl, workspaceRoot) {
  let html;
  if (tasks.length === 0) {
    html = `
    ...
    const taskRows = tasks.map((t) => {
      ...
      if (runInfo && (runInfo.status === "queued" || runInfo.status === "in_progress")) {
        ...
        statusPill = t.enabled ? `<span class="schedule-pill schedule-pill-on">Active</span>` : `<span class="schedule-pill schedule-pill-off">Paused</span>`;
      }
      ...
      const runNowBtn = canRun ? isRunning ? `<button class="schedule-action-btn schedule-action-running" disabled title="Running\u2026"><span class="codicon codicon-loading codicon-modifier-spin"></span></button>` : `<button class="schedule-action-btn schedule-action-run" data-command="runTask" data-file="${escAttr(t.id)}" title="${wfExists ? "Run on GitHub Actions" : "Run now"}"><span class="codicon codicon-rocket"></span></button>` : "";
      const viewRunBtn = runInfo?.runUrl ? `<button class="schedule-action-btn" data-command="openExternal" data-file="${escAttr(runInfo.runUrl)}" title="View run on GitHub"><span class="codicon codicon-link-external"></span></button>` : "";
      const editPromptBtn = t.mode === "agent" && t.promptFile ? `<button class="schedule-action-btn" data-command="openPromptFile" data-file="${escAttr(t.promptFile)}" title="Edit prompt"><span class="codicon codicon-edit"></span></button>` : "";
      ...
      const lastRunHtml = taskState[t.id]?.lastRun ? `<span class="schedule-last-run" title="Last run: ${escAttr(taskState[t.id].lastRun)}"><span class="codicon codicon-history"></span> ${timeAgo(taskState[t.id].lastRun)}</span>` : "";
      let depsHtml = "";
...
function loadTaglineConfig(workspaceRoot, fs11, path11) {
  const configPath = path11.join(workspaceRoot, ".github", "config", "taglines.json");
...
function renderAgentActivity(_workspaceRoot) {
...
function renderPRRow(pr) {
  ...
  <div class="agent-pr-row" data-command="openExternal" data-file="${escAttr(pr.url)}" role="button" tabindex="0" title="${escAttr(pr.title)}">
    ...
    <div class="agent-pr-info">
      ...
      <span class="agent-pr-meta">#${pr.number} \xB7 ${escHtml(timeAgo2(pr.createdAt))} \xB7 ${escHtml(pr.author)}</span>
    </div>
...
<body>
  ...
  <div id="tab-autopilot" class="tab-panel" role="tabpanel" aria-labelledby="tab-btn-autopilot">
    ...
    ${renderScheduledTasks(this.workspaceRoot ? loadScheduledTasks(this.workspaceRoot) : [], this.workspaceRoot ? getGitHubRepoUrl(this.workspaceRoot) : void 0, this.workspaceRoot)}
  </div>
...
function detectProject(workspaceRoot) {
  ...
  const commands4 = extractCommands(workspaceRoot, pkg);
  let projectType = "generic";
  if (pkg) {
    const engines = pkg.engines;
    if (engines?.vscode) {
      projectType = "vscode-extension";
      return { projectType, ...commands4, conventions };
    }
  }
...
async function enforceSafetySettings() {
  const config = vscode7.workspace.getConfiguration();
  const key = "github.copilot.chat.copilotMemory.enabled";
  if (config.get(key) !== false) {
    await config.update(key, false, vscode7.ConfigurationTarget.Global);
  }
}
function activate(context) {
  enforceSafetySettings();
  initRunStore(context.workspaceState);
  const participant = vscode7.chat.createChatParticipant(
    "alex.chat",
    chatHandler
  );
  participant.iconPath = vscode7.Uri.joinPath(
  ...
  const statusTimer = setInterval(() => {
    updateAgentStatusBar(vscode7.workspace.workspaceFolders?.[0]?.uri.fsPath);
  }, 5 * 60 * 1e3);
  ...
  context.subscriptions.push(
    vscode7.window.createTreeView("alex.agentActivity", {
      treeDataProvider: agentActivityProvider
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.refreshAgentActivity", () => {
      agentActivityProvider.refresh();
    })
  );
  if (workspaceRoot) {
    const metricsPattern = new vscode7.RelativePattern(workspaceRoot, ".agent-metrics-state.json");
    const metricsWatcher = vscode7.workspace.createFileSystemWatcher(metricsPattern);
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.refreshWelcome", () => {
      welcomeProvider.refresh();
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.openChat", () => {
      vscode7.commands.executeCommand("workbench.action.chat.open");
    })
  );
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.dream", async () => {
      try {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.initialize", async () => {
      try {
        await enforceSafetySettings();
        const memPath = resolveAIMemoryPath();
        if (!memPath) {
          const setup = await vscode7.window.showInformationMessage(
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.upgrade", async () => {
      try {
        await enforceSafetySettings();
        const memPath = resolveAIMemoryPath();
        if (!memPath) {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.setupAIMemory", async () => {
      try {
        await setupAIMemory();
        welcomeProvider.refresh();
      } catch (err) {
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.generateLoopConfig", async () => {
      ...
      try {
        ...
        if (ok) {
          ...
          vscode7.window.showErrorMessage("Alex: Failed to write loop config.");
        }
      } catch (err) {
        vscode7.window.showErrorMessage(
  ...
  context.subscriptions.push(
    vscode7.commands.registerCommand("alex.setProjectPhase", async () => {
      const wsRoot = vscode7.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (!wsRoot) {
        vscode7.window.showWarningMessage("Alex: Open a workspace folder first.");
        return;
  ...
  const loopConfigWatcher = vscode7.workspace.createFileSystemWatcher(
    new vscode7.RelativePattern(
      vscode7.workspace.workspaceFolders?.[0] ?? "",
      ".github/config/loop-menu.json"
    )
  );
  const loopPromptWatcher = vscode7.workspace.createFileSystemWatcher(
    new vscode7.RelativePattern(
      vscode7.workspace.workspaceFolders?.[0] ?? "",
      ".github/prompts/loop/*.prompt.md"
    )
  );
  const skillPartialWatcher = vscode7.workspace.createFileSystemWatcher(
    new vscode7.RelativePattern(
      vscode7.workspace.workspaceFolders?.[0] ?? "",
