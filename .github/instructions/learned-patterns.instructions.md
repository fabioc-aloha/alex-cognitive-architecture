...
- **DryRun must simulate non-existent paths**: When a script renames then restores, dry-run skips the rename so the restore source doesn't exist. Solution: `$restoreSource = if ($DryRun) { $originalPath } else { $backupPath }`. Test dry-run separately from real runs.
- **Two-phase fleet upgrade**: Phase 1 = mechanical batch (`upgrade-brain.ps1 -Mode Full`). Phase 2 = semantic curation per-project (scan `.github-backup-*/` for custom content worth restoring). Batch for correctness, human for judgment.
- **Brain source path**: `platforms/vscode-extension/.github/` is the canonical brain source. `heir/.github/` is stale and divergent — never use as source.
- **Three "hooks" in AlexMaster**: (1) `.github/hooks/` = git pre-commit hooks (master-only), (2) `.github/muscles/hooks/` = agent hook scripts (distributed), (3) `.github/config/hooks.json` = agent hook event registry. BRAIN_SUBDIRS must NOT include `hooks`.
