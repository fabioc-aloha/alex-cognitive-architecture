...
- **Two-phase fleet upgrade**: Phase 1 = mechanical batch (`node scripts/upgrade-brain.cjs --mode Full`). Phase 2 = semantic curation per-project (scan `.github-backup-*/` for custom content worth restoring). Batch for correctness, human for judgment. The `.github/muscles/retro-tag-inheritance.cjs` muscle (inheritable, ships with every heir) classifies the backup directory into `inheritable`/`custom`/`master-only` so Phase 2 only reviews the `custom` set instead of every file.
