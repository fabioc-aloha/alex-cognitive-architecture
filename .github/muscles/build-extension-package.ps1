...
.DESCRIPTION
    Orchestrates a full extension build:
      ...
      6. Stage marketplace files (README, CHANGELOG from heir)
      7. VSIX package creation (via vsce, triggers vscode:prepublish pipeline)
      8. VSIX output verification
...
# ------------------------------------------------------------
# Step 6: Stage Marketplace Files
# ------------------------------------------------------------
Write-Host "[6/8] Staging marketplace files from heir..." -ForegroundColor Yellow
$heirRoot = Join-Path $RootPath "heir"

# Copy README.md from heir (marketplace landing page)
$heirReadme = Join-Path $heirRoot "platforms" "vscode-extension" "README.md"
if (-not (Test-Path $heirReadme)) {
    $heirReadme = Join-Path $heirRoot "README.md"
}
if (Test-Path $heirReadme) {
    if (-not $DryRun) {
        Copy-Item $heirReadme (Join-Path $ExtensionPath "README.md") -Force
    }
    Write-Host "  [OK] README.md staged" -ForegroundColor Green
}
else {
    Write-Host "  [WARN] No README.md found in heir" -ForegroundColor Yellow
}

# Copy CHANGELOG.md from heir (marketplace changelog)
$heirChangelog = Join-Path $heirRoot "CHANGELOG.md"
if (Test-Path $heirChangelog) {
    if (-not $DryRun) {
        Copy-Item $heirChangelog (Join-Path $ExtensionPath "CHANGELOG.md") -Force
    }
    Write-Host "  [OK] CHANGELOG.md staged" -ForegroundColor Green
}
else {
    Write-Host "  [WARN] No CHANGELOG.md found in heir" -ForegroundColor Yellow
}

# ------------------------------------------------------------
# Step 7: Create VSIX Package
# ------------------------------------------------------------
Write-Host "[7/8] Creating VSIX package..." -ForegroundColor Yellow
if ($SkipCompile) {
...
elseif ($DryRun) {
    ...
    try {
        ...
        Write-Host "  (triggers vscode:prepublish → esbuild --production)" -ForegroundColor DarkGray
...
# ------------------------------------------------------------
# Step 8: Verify VSIX Output
# ------------------------------------------------------------
Write-Host "[8/8] Verifying VSIX..." -ForegroundColor Yellow
